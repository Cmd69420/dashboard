...
const JourneyTrackingPage = () => {
  ...
  const [viewMode, setViewMode] = useState("summary"); // summary, detailed, map
  ...
  const fetchJourneyData = async () => {
    ...
    try {
      // Fetch location logs for the date range
      const logsResponse = await fetch(
        `${API_BASE_URL}/admin/location-logs/${selectedUser.id}?limit=10000`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const meetingsResponse = await fetch(
        `${API_BASE_URL}/admin/user-meetings/${selectedUser.id}?limit=1000`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
