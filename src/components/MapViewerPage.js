      ...
      const locationPromises = (usersData.users || []).map(async (user) => {
        try {
          const logsRes = await fetch(
            `${API_BASE_URL}/admin/location-logs/${user.id}?limit=1`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          ...
          return {
            ...
            user: user,
            location: latestLog,
            clockedIn: clockData.clocked_in,
            lastSeen: clockData.last_seen,
          };
        } catch (err) {
          return { userId: user.id, user: user, location: null, clockedIn: false };
        }
      });
